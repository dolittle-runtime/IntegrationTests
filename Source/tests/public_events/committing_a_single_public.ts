...
export class committing_a_single_public extends ScenarioFor<a_producer_and_a_consumer> {
    ...
    then_event_should_be_in_the_event_handler_stream_of_the_consumer_microservice = () => this.context?.consumer?.streams?.should_contain(this.context.tenant, EventHandlers.publicEventHandlerId, Scopes.producerScope, this.event_committed);
    then_event_should_be_in_the_unpartitioned_event_handler_stream_of_the_consumer_microservice = () => this.context?.consumer?.streams?.should_contain(this.context.tenant, EventHandlers.unpartitionedPublicEventHandlerId, Scopes.producerScope, this.event_committed);
    then_external_event_handler_should_have_handled_event = () => this.context?.consumer?.stream_processors?.should_have_event_handler_at_position(this.context?.tenant, EventHandlers.publicEventHandlerId, 1, Scopes.producerScope);
    then_external_unpartitioned_event_handler_should_have_handled_event = () => this.context?.consumer?.stream_processors?.should_have_event_handler_at_position(this.context?.tenant, EventHandlers.unpartitionedPublicEventHandlerId, 1, Scopes.producerScope);
}
