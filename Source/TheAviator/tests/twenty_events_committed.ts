...
export class twenty_events_committed extends scenario_for_a_single_microservice {
    ...
    then_all_events_should_be_in_stream_for_processor = () => this.streams?.should_contain(this.tenant, EventHandlers.eventHandlerId, ...this._events);
    then_event_handler_should_have_handled_all = () => this.stream_processors?.should_have_event_handler_at_position(this.tenant, EventHandlers.eventHandlerId, 20);
}